...
--and tblname in('efpgprocesso')
AND tblpages - est_tblpages_ff > 0
AND (bs*tblpages)::bigint > 1000000000 -- > 1000 MB
and ( CASE WHEN tblpages - est_tblpages_ff > 0 THEN (100 * (tblpages - est_tblpages_ff)/tblpages::float)::numeric(50,2) ELSE 0 END) >= 25
 AND (100 * (tblpages - est_tblpages_ff)/tblpages::float) > 25  -- bloat > 30 %
...
--AND tblname ~* '^(hrcnegociacao_parcela_aux|hrctab_log|hrccredito_problematico)
