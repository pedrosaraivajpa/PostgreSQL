...
| `06_monitore_cpu.sql` | Função PL/Python para monitorar uso de CPU por processo PostgreSQL + script shell de coleta | PG 13+ (plpython3u) |
| `07_identifica_vacuum_aggressive.sql` | Identifica tabelas próximas do limite de vacuum agressivo (transaction ID wraparound) | PG 9.4 |
...
| CPU alta no servidor de banco | `06` |
| Alerta de transaction ID wraparound | `07` |
...
- `06_monitore_cpu.sql` requer a extensão `plpython3u` e o pacote Python `psutil` instalados no servidor
- `07_identifica_vacuum_aggressive.sql` tem um filtro de schema — ajuste conforme necessário
- O script de bloat (`05`) tem filtros de tamanho mínimo comentados — ajuste conforme o ambiente
